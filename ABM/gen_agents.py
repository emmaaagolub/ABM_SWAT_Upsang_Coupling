...
def generate_farmer(agent_num,patch_attribute,farmer_attribute, patch_ini_state, farmer_ini_state, adjacency_matrix, neighbor_influence_matrix):
    ...
    for idx in range(agent_num):
        ...
        Attributes = {'discount_factor': temp[1], 'risk_factor': temp[2],'farm_size': farm_size, 'info_use':temp[3],
                      ...
                      'community_ID': int(temp[11]),'cost_adj_factor':temp[12],'type':temp[13],'sensi_community': temp[14], 'tao_factor': temp[15],
                      'neighbors': neighbors, 'neigh_weight': neighbor_weights,'patch_ID': patch_IDs,'patch_areas': Areas,
                      'patch_CRP_eligible': CRP_eligible, 'patch_slope': slopes, 'soil_loss': soil_loss}
        ...
        States = {'price_prior': [temp[1:7]], 'price_post': [], 'price_received': [], 'climate_forecasts': [[temp[7],temp[8],0.1]],
                  ...
                  'SC_Will':[], 'SC_Ratio':[], 'is_adopt':[],'learning_factor':[0]}
...
def gen_community(agent_num,community_attribute,community_ini_state): # COME BACK TO THIS LINE TO ADJUST COMMUNITIES FOR UPPER SANGAMON ONLY
