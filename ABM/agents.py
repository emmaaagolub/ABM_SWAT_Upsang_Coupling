...
survey_data_loc = os.path.join(params.dir_loc,"BN_rule")
survey_data = pd.read_csv(os.path.join(survey_data_loc,"BN_data_start1_adj.csv"))
bn_df=pd.read_csv(os.path.join(survey_data_loc,"BN_df.csv")) # import the MC samples for BN inferencing
...
class Farmer:
    ...
    def cal_max_potential_profit_peren(self,prices,contract_feed_type,contract_max_price,year):
        ...
        environ_factor = self.States['learning_factor'][-1]
        ...
        max_profit = np.max(profit_peren) + config.mis_carbon_credit * gov.carbon_price[year] + config.mis_N_credit * gov.nitrogen_price
    ...
    def identify_contract_patches(self,community_list,contract_feed_type,contract_max_price,year):
        ...
        for i in self.Attributes['adopt_priority']:
            if (exist_contract[i]>0) & (Peren_ages[i]<GMP.cal_contract_length(land_uses_b[i])):
                ...
                if self.Attributes['type'] in [1,2,3,4]: # Type I farmers allow adoption of perennial crop when it is profitable, the other two types do not
                    if (argmax_U_peren > 1) & (max_U_peren > 10.*U_max_crop) & (SC_Rev_mean_num > 400 *2.47):
                        is_contract[i] = 1
                        opt_land_use[i] = peren_lookup_table[int(argmax_U_peren)]
                    elif (max_U_peren > 10.*U_max_crop) & (SC_Rev_mean_num > 400 *2.47):
                        opt_land_use[i] = peren_lookup_table[int(argmax_U_peren)]
    ...
    def land_use_decision_econo(self,is_contract,year):
        ...
        for idx in range(N_patch):
            policy_eligibilities = [BCAP_eligible[idx], TMDL_eligible[idx], CRP_eligible[idx],
                                    environ_factor]  # since this is only for economic reasoning, soil_loss is not taken into account
            if (Peren_ages[idx] >= GMP.cal_contract_length(land_uses_b[idx])) | (Peren_ages[idx] < 0): # !!!! if it is not already growing perennial grass
                ...
                best_peren[idx], U_max_peren[idx] = self.cal_max_potential_U_peren(idx, land_uses_b[idx], climates, prices, Peren_ages[idx], policy_eligibilities,
                                      contract_feed_type, contract_max_price,year)
                U_fallow = -loss_aver*(-areas[idx]*(0+policy_eligibilities[1]*gov.TMDL_subsidy -
                                       config.land_rent * (1-self.Attributes['patch_CRP_eligible'][idx] * config.marginal_land_rent_adj)))**risk_aver
                if policy_eligibilities[2]>0:
                    if (0 + gov.CRP_subsidy + policy_eligibilities[1]*gov.TMDL_subsidy -
                        config.land_rent* (1-self.Attributes['patch_CRP_eligible'][idx] * config.marginal_land_rent_adj)) > 0:
                        U_CRP = (areas[idx] * (0 + gov.CRP_subsidy + policy_eligibilities[1]*gov.TMDL_subsidy -
                                 config.land_rent * (1-self.Attributes['patch_CRP_eligible'][idx] * config.marginal_land_rent_adj))) ** risk_aver
                    else:
                        U_CRP = -loss_aver*(-areas[idx]*(0 + gov.CRP_subsidy + policy_eligibilities[1] * gov.TMDL_subsidy -
                                              config.land_rent * (1-self.Attributes['patch_CRP_eligible'][idx] * config.marginal_land_rent_adj))) ** risk_aver
                else:
                    U_CRP = -10**6

                if self.Attributes['type'] in [1,2,3,4]: # Type I farmers allow adoption of perennial crop if it is profitable, the other two Types do not
                    SC_Rev_mean_num, opt_peren = self.cal_max_potential_profit_peren(prices, contract_feed_type,
                                                                                     contract_max_price, year)
                    if SC_Rev_mean_num > 400 * 2.47:
                        best_ID = np.argmax([U_max_crop[idx],U_max_peren[idx]/10.,U_fallow,U_CRP])
                    else:
                        best_ID = np.argmax([U_max_crop[idx], -10**6, U_fallow, U_CRP])
            ...
            elif (is_contract[idx] < 1):
                profit_peren = GME.cal_profit_exp_patch(self.Attributes['patch_ID'][idx], land_uses_b[idx], 3,
                                                        self.Attributes['patch_slope'][idx],
                                                        Peren_ages[idx], policy_eligibilities,
                                                        self.Attributes['soil_loss'][idx],
                                                        prices, self.Attributes['discount_factor'], [0, 0, 0], year)
                profit_corn = GME.cal_profit_exp_patch(self.Attributes['patch_ID'][idx], land_uses_b[idx],
                                                       1, self.Attributes['patch_slope'][idx], Peren_ages[idx],
                                                       policy_eligibilities, self.Attributes['soil_loss'][idx],
                                                     prices, self.Attributes['discount_factor'], [0, 0, 0], year)
                if profit_peren < profit_corn:
                    opt_land_use[idx] = 1
                else:
                    opt_land_use[idx] = land_uses_b[idx]
            else: # if already having a contract
                opt_land_use[idx] = land_uses_b[idx]
    ...
    def learn_from_experience(self,year):
        # function for farmer to update their perception of longterm economic benefits of miscanthus
        land_use = self.States['land_use'][-1]
        patch_ID = self.Attributes['patch_ID']
        h_in = self.States['learning_factor'][-1] # the factor reflecting farmer's perception of miscanthus's longterm benefits
        tao = self.Attributes['tao_factor'] # the factor representing farmer's learning rate
        mis_ID = []
        for i in range(land_use.size):
            if (land_use[i]>=3)&(land_use[i]<=6):
                mis_ID.append(i)
        ...
        if mis_ID.__len__() == 0:
            ...
            S = []
            for i in range(mis_ID.__len__()):
                S.append(config.patch_N_loads[year][patch_ID[mis_ID[i]],land_use[mis_ID[i]]]/
                         config.patch_N_loads[year][patch_ID[mis_ID[i]],1])
            S = 1-np.mean(np.asarray(S))
            if S >= 0:
                h_out = h_in + S*tao*(1-h_in)
            else:
                h_out = h_in + S*tao*h_in

        h_out = max(0,h_out)
        h_out = min(1,h_out)
        self.States['learning_factor'].append(h_out)
        return h_out

    def learning_from_neighbor(self,learning_factor_neighbors):

        # ID = self.ID
        neigh_weight = self.Attributes['neigh_weight']
        neighbors = self.Attributes['neighbors']
        learning_factor = np.dot(neigh_weight,learning_factor_neighbors[neighbors]) # final learning_factor

        learning_factor = max(0, learning_factor)
        learning_factor = min(1, learning_factor)
        self.States['learning_factor'][-1] = learning_factor  # update farmer's attitude
    ...
    def bn_inference(self,AT_commu,sign_AT,SC_Contract,SC_Rev_mean,year):
        ...
        if (SC_Contract == 1) & (SC_Rev_mean==3):
            df_temp = bn_df[(bn_df['farm_size'] == self.Attributes['farm_size']) &
                            (bn_df['info_use'] == self.Attributes['info_use']) &
                            (bn_df['peer_ec'] == self.States['peer_ec'][-1]) &
                            (bn_df['benefit'] == self.Attributes['benefit']) &
...
class Can_ref:
    ...
    def cal_NPV(self, feed_prices, product_prices, price_adj, trans_costs, storage_costs, subsidys,taxs,ref_inv_cost_adj_his,ref_pro_cost_adj_his,year):
        ...
        NPV = npf.npv(config.inflation_rate,cash_flow)
        ...
        for farm_agent in farmer_list:
            ...
            if commu_ID == ID:
                ...
                for i in range(patch_ID.size):
                    ...
                    print("sum of N loads is ",N_loads_sum)
                    print("areas sum is ",areas_sum)

        epsilon = 1e-10  # A small positive value to avoid division by very small numbers
        average_N_loads = N_loads_sum / (areas_sum + epsilon)
...
class govern_agent:
    ...
    def cal_IRR_adj_factor(self,RFS_signal):
        ...
        if RFS_signal[-1] == -1:
            IRR_adj_factor = config.IRR_adj_rate
    ...
    def make_contracts(self,farmer_list,ref_list,feed_prices,trans_costs,storage_costs,PBE,market_demand,market_supply,community_list,year):
        ...
        if if_new_contract == 0:
            ...
            for idx in np.random.permutation(N_neighbors):
                ...
                if contract_max_price > config.external_provided_prices[:, 10][0] + config.system_boundary_radius * config.trans_costs[2]:
                    contract_max_price = config.external_provided_prices[:, 10][0] + config.system_boundary_radius * config.trans_costs[2]
                LB = market_feedstock_price # farmer's minimum acceptable price
                # LB = 1000000   # temporally disable making contracts
