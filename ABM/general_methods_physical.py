...
import config
import copy
import biorefineries.abm.corn as abm_1 # make sure the order of .abm.corn is correct!
import biorefineries.abm.cornstover as abm_2 # make sure the order of .abm.cornstover is correct!
import pandas as pd
import os
...
def quick_cal_ref_feed_amount(can_ref_agent,farmer_list,bagasse_avaiable,delta_LU_limit,occupied_feed):
    ...
    for ind in ID_within_range:
        ...
        for i in range(N_patch):
            ...
            if (farmer_agent.Attributes['patch_CRP_eligible'][i] == 1) & (land_use != 3):  # accumulate all the marignal lands that is not growing miscanthus
                marginal_area += area
    ...
    marginal_ratio = marginal_area/land_use_areas.sum()
    delta_LU_limit += marginal_ratio  # add marignal lands to the allowable land use change
    ...
    if (max(productions[0:2]) >= can_ref_agent.Attributes['capacity'])|(productions[5] > can_ref_agent.Attributes['capacity']): # if the current available feedstock can support the refinery, use the current ones
        ...
        if (max(productions_adj[0:2]) >= can_ref_agent.Attributes['capacity']/(1-config.mis_trans_loss-config.mis_storage_loss))|\
            ...
            if tech_type < 5:
                ...
                feed_amount = (feed_available_adj - occupied_feed_loc) * can_ref_agent.Attributes['capacity'] / productions_adj[5]
...
def cal_contract_length(land_uses):
    ...
    for i in range(N):
        if (land_uses[i] == np.asarray([1,2])).sum()>0:
            ...
            contract_length[i] = 15
        elif (land_uses[i] == np.asarray([1,4])).sum()>0:
            contract_length[i] = 10
...
def collecting_farmer_learning_factor(farmer_list):
    # function to compile of all farmers' perception of miscanthus's longterm benefits
    N_farmer = farmer_list.__len__()
    learning_factor = np.zeros(N_farmer)
    for i in range(N_farmer):
        learning_factor[i] = copy.deepcopy(farmer_list[i].States['learning_factor'][-1])
