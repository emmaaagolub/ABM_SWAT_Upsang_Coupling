...
def cal_profit_exp_patch(patch_ID,land_use_b,land_use_n,slope,peren_age,policy_eligibilities,soil_loss,prices,discount_rate,risks,year):
    ...
    if land_use_n == 7:
        ...
        cost = 0
        profit = (revenue - cost) + is_environmental * soil_loss - config.land_rent * (1-CRP_eligible * config.marginal_land_rent_adj)
    elif land_use_n == 1:
        revenue = yield_patch * price
        cost = fix_cost + via_cost * yield_patch + ferti_use * config.ferti_price + config.land_rent * (1-CRP_eligible * config.marginal_land_rent_adj)
        ...
        cost_rotation_0 = fix_cost + via_cost * yield_patch_rotation_0 + ferti_use * config.ferti_price + \
                          config.land_rent * (1-CRP_eligible * config.marginal_land_rent_adj)
        cost_rotation_1 = config.farmer_cost_table[0, 1] + config.farmer_cost_table[1, 1] * yield_patch_rotation_1 + \
                          config.fertilizer_table[1] * config.ferti_price + config.land_rent * (1-CRP_eligible * config.marginal_land_rent_adj)
        ...
        annual_revenue = yield_patch * price + BCAP_eligible * gov.BCAP_subsidy + TMDL_eligible * gov.TMDL_subsidy + \
                         CRP_eligible * gov.CRP_subsidy * gov.CRP_relax + config.mis_carbon_credit * gov.carbon_price[year] + \
                         config.mis_N_credit * gov.nitrogen_price
        annual_cost = fix_cost + via_cost * yield_patch + ferti_use * config.ferti_price + config.land_rent * (1-CRP_eligible * config.marginal_land_rent_adj)
        ...
        half_cost = fix_cost + config.perennial_yield_adj_table[1, 0] * via_cost * yield_patch + \
                    ferti_use * config.ferti_price + config.land_rent * (1-CRP_eligible * config.marginal_land_rent_adj)
        if is_failed == 0: # if the installation does not fail
            ...
            profit = npf.pmt(discount_rate,15,-NPV,0) + is_environmental * soil_loss
        else:
...
def cal_BEM_patch(farmer_list,N_patch,N_crop,market_prices,empirical_risks,year):
    ...
    patch_attribute = np.loadtxt(os.path.join(params.dir_loc,"data","patch_attributes.csv"), delimiter=',', skiprows=1) # column 3 contains the marginal land information
    # ^BUT WHAT IS THIS CURRENT WORKING DIRECTORY SUPPOSED TO BE? params.folder or just main data folder? There's two...
    current_profit_patch = np.zeros((N_patch,N_crop+2))
    current_profit_patch[:,-2] = 0
    current_profit_patch[:,-1] = copy.deepcopy(gov.CRP_subsidy-config.land_rent * (1-patch_attribute[:,3] * config.marginal_land_rent_adj))
...
def cal_bargain_price(LB,UB,market_demand,market_supply):
    ...
    fn = lambda x: LB + (UB-LB)**(market_tightness * x * lambda_market) - UB + (UB-LB)**((1-market_tightness) * x * lambda_market)
    ...
    P_bargain = LB + (UB-LB)**(market_tightness * bid_step * lambda_market)

    return P_bargain

def cal_market_price(ori_price,market_demand,market_supply,lambda_market):
    # function to update the market price of biomass
    if (market_demand < 500000) & (market_demand <= market_supply):
        excess_demand = 0
    else:
        excess_demand = (market_demand-market_supply)/market_demand
    market_price = ori_price * np.exp(excess_demand/lambda_market)
    return market_price

def update_market_feed_price(ref_list,farmer_agent_list,feed_price_external,feed_price,product_prices_pred):
    ...
    for i in range(N_ref):
        if ref_list[i].Attributes['tech_type'] > 1:
            ...
            market_demand.append(ref_list[i].Attributes['feedstock_amount'][2] + ref_list[i].Attributes['feedstock_amount'][3] -
    ...
    LB = feed_price_external
    if PBE.__len__()==0:
        UB = LB+0.000001
    else:
        UB = (np.asarray(PBE) * np.asarray(market_demand)).sum() / np.asarray(market_demand).sum()

    if UB > config.external_provided_prices[:, 10][0] + config.system_boundary_radius * config.trans_costs[2]:
        UB = config.external_provided_prices[:, 10][0] + config.system_boundary_radius * config.trans_costs[2]
    ...
    # feed_price = cal_market_price(feed_price,market_demand, market_supply,config.lambda_market)
