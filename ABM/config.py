...
def rewrite_farmer_composition(farmer_change_rate):
    ...
    agent_cluster_ABM = pd.read_excel(os.path.join(params.dir_loc,"BN_rule","agent_cluster_ABM.xlsx")) 
    ...
    farmer_attribute = pd.read_csv('./data/farmer_attributes_backup.csv') #filtered_farmer_attributes_backup
    farmer_ini = pd.read_csv('./data/farmer_ini_state_backup.csv') #filtered_farmer_ini_state_backup
    ...
    farmer_attribute.to_csv('./data/farmer_attributes.csv',index=False) #filtered_farmer_attributes
    farmer_ini.to_csv('./data/farmer_ini_state.csv', index=False) #filtered_farmer_ini_state
...
patch_attribute = np.loadtxt('./data/patch_attributes.csv', delimiter=',', skiprows=1) #filtered_patch_attributes
farmer_attribute = np.loadtxt('./data/farmer_attributes.csv', delimiter=',', skiprows=1) #filtered
patch_ini_state = np.loadtxt('./data/patch_ini_state.csv', delimiter=',', skiprows=1) #filtered
farmer_ini_state = np.loadtxt('./data/farmer_ini_state.csv', delimiter=',', skiprows=1) #filtered
adjacency_matrix = np.loadtxt('./data/adjacency_matrix.csv', delimiter=',', skiprows=0) #filtered
neighbor_influence_matrix = np.loadtxt('./data/neighbor_influence_matrix.csv', delimiter=',', skiprows=0) #filtered
...
lambda_market = params.lambda_market # the lambda parameter for updating biomass market price
...
patch_N_loads = []
for i in range(30):
    patch_N_loads.append(np.loadtxt('./data/resp_matrix/patch_N_loads_'+str(i+1)+'.csv',delimiter=',',skiprows = 1, usecols=(1,2,3,4,5,6,7,8))) #filtered
...
ref_farmer_dist_matrix = np.loadtxt('./data/ref_farmer_dist_matrix.csv',delimiter=',',skiprows = 0) #filtered
...
BCHP_farmer_dist_matrix = np.loadtxt('./data/BCHP_farmer_dist_matrix.csv',delimiter=',',skiprows = 0) #filtered
cofire_farmer_dist_matrix = np.loadtxt('./data/cofire_farmer_dist_matrix.csv',delimiter=',',skiprows = 0) #filtered
...
system_boundary_radius = 110
refinery_esti_farmer_discount_rate = 0.08
allowable_defecit = params.allowable_defecit
refinery_life_span = 50
refinery_delta_LU_limit = 0.2
max_ref_cap = 0.6*10**9  # maximum capacity of a single refinery in L ethanol
...
IRR_adj_rate = params.IRR_adj_rate
