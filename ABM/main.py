# MAKE SURE IN CMD TERMINAL TO CHANGE TO YOUR DIRECTORY AND ACTIVATE .ABMVENV TO ENSURE PROPER PACKAGE INITIALIZATION. 
# This can be done by: 
# cd {desired\directory\}
# .abmvenv\Scripts\activate
# python main.py

# You will be prompted to select a scenario to run for both a parameter.py file and the main.py file. Make sure they are matching.
...
#dir_loc = r"C:\Users\egolub2\BiofuelABM\ABM_insight_model_OG"
...
def scenario_bioenergy_baseline():
    imp.reload(params)
    params.result_file = './results/ABM_result_bioenergy_baseline.out'
    params.maintain_RFS = 1
    params.base_cap_for_learn_by_do = 200 * 10 ** 6
    imp.reload(gov)
    imp.reload(config)
    gov.gov_ref_tax_subsidy[4:, 2] = 0
    gov.gov_ref_tax_subsidy[4:, 3] = 0
    gov.gov_ref_tax_subsidy[4:, 5] = 0.21
    gov.ref_subsidys[4:, 1] = 0
    gov.ref_subsidys[4:, 2] = 0
    gov.ref_taxs[4:,1] = 0.21
    config.max_watersh_cap_cell = 3.0 * 10 ** 9
    main()

def scenario_small_scale_biofacility():
    imp.reload(params)
    params.result_file = './results/ABM_result_bioproduct_baseline.out'
    params.maintain_RFS = 1
    params.base_cap_for_learn_by_do = 200 * 10 ** 6
    imp.reload(gov)
    imp.reload(config)
...
def scenario_environ_only():
    imp.reload(params)
    params.result_file = './results/ABM_result_environ_only_baseline.out'
    params.maintain_RFS = 1
    params.base_cap_for_learn_by_do = 200 * 10 ** 6
    params.CRP_relax = 1
    imp.reload(gov)
    ...
    gov.ref_taxs[4:,1] = 0.21
    config.max_watersh_cap_cell = 3.0 * 10 ** 9
    gov.carbon_price[:11] = 0.3 * np.linspace(0,100,11)
    gov.carbon_price[11:] = 0.3 * 100
    main()

def scenario_environmental_benefit():
    imp.reload(params)
    params.result_file = './results/ABM_result_environ_baseline.out'
    params.maintain_RFS = 1
    params.base_cap_for_learn_by_do = 200 * 10 ** 6
    params.CRP_relax = 1
    imp.reload(gov)
    imp.reload(config)
    config.max_watersh_cap_cell = 5.0 * 10 ** 9
    gov.carbon_price[:11] = 0.3 * np.linspace(0,100,11)
    gov.carbon_price[11:] = 0.3 * 100
    main()

def scenario_environmental_benefit_with_TMDL():
    imp.reload(params)
    params.result_file = './results/ABM_result_environ_baseline_TMDL.out'
    params.maintain_RFS = 1
    params.base_cap_for_learn_by_do = 200 * 10 ** 6
    params.CRP_relax = 1
    imp.reload(gov)
    ...
    gov.carbon_price[:11] = 0.3 * np.linspace(0,100,11)
    gov.carbon_price[11:] = 0.3 * 100
    params.TMDL_subsidy = 60 * 2.47 #60$/acre uses hectare for compiliation
    main()

def scenario_environmental_benefit_with_CRP_miscanthus_cultivation_allowed():
    imp.reload(params)
    params.result_file = './results/ABM_result_environ_baseline_CRP_allow.out'
    params.maintain_RFS = 1
    params.base_cap_for_learn_by_do = 200 * 10 ** 6
    params.CRP_relax = 1 # perennial grass harvesting on CRP ALLOWED
    imp.reload(gov)
    imp.reload(config)
    config.max_watersh_cap_cell = 5.0 * 10 ** 9
    gov.carbon_price[:11] = 0.3 * np.linspace(0,100,11)
...
def scenario_environmental_benefit_with_CRP_subsidy():
    imp.reload(params)
    params.result_file = './results/ABM_result_environ_baseline_CRP.out'
    params.maintain_RFS = 1
    params.base_cap_for_learn_by_do = 200 * 10 ** 6
    params.CRP_relax = 1 # perennial grass harvesting on CRP ALLOWED
    imp.reload(gov)
    ...
    gov.carbon_price[:11] = 0.3 * np.linspace(0,100,11)
    gov.carbon_price[11:] = 0.3 * 100
    params.CRP_subsidy = 300    #$/ha
    main()

# Define a dictionary mapping scenarios to functions
scenario_functions = {
    'S1': scenario_bioenergy_baseline,
    'S1.1': scenario_small_scale_biofacility,
    'S2': scenario_environ_only,
    'S3': scenario_environmental_benefit,
    'S4': scenario_environmental_benefit_with_TMDL,
    'S5': scenario_environmental_benefit_with_CRP_subsidy,
...
def main():
    ...
    # farmer
    N_farmer = 1000
    patch_attribute = config.patch_attribute
    farmer_attribute = config.farmer_attribute
    patch_ini_state = config.patch_ini_state
    farmer_ini_state = config.farmer_ini_state
    adjacency_matrix = config.adjacency_matrix
    neighbor_influence_matrix = config.neighbor_influence_matrix
    ...
    for year in range(config.simu_horizon):
        print(year)
        # step zero is to update the biomass market price and the policy information to each farmer agent
        ...
        mis_price_market, market_demand_watershed, market_supply_watershed = GME.update_market_feed_price(ref_list,
                                                                    farmer_agent_list,config.external_provided_prices[year, 9],
                                                                    feed_prices[2],product_prices_pred)
        feed_prices = copy.deepcopy(config.external_provided_prices[year, 7:])
        ...
        for temp_farmer in farmer_agent_list:  # update farmer's revenue
            ...
            revenue_temp += (temp_farmer.States['land_use'][-1] == 3) * (
                        config.mis_carbon_credit * gov.carbon_price[year] + config.mis_N_credit * gov.nitrogen_price) + \
            ...
            temp_farmer.learn_from_experience(year)

        learning_factor_neighbors = GMP.collecting_farmer_learning_factor(farmer_agent_list)  # farmer's perception of mischanthus's longterm benefit
        for temp_farmer in farmer_agent_list:  # update farmer's perception based on neighborhood learning
            temp_farmer.learning_from_neighbor(learning_factor_neighbors)
            #
            price_posterior = temp_farmer.forecasts_price(temp_farmer.States['price_prior'][-1],
                                                          feed_prices[np.asarray([0,1,2,3,5,6])])
    ...
    # path = os.path.join(params.folder, "results")

    # try:
    #     os.makedirs(os.path.join(params.folder, path))
    # except OSError:
    ...
    # Store local variables in a shelf
    my_shelf = shelve.open(params.result_file, 'n')  # 'n' for new

    for key in dir():
        if key not in ['my_shelf', '__builtins__', 'shelve'] and \
        not callable(locals()[key]) and \
        not isinstance(locals()[key], (type, types.ModuleType)):
            try:
                my_shelf[key] = locals()[key]
            except TypeError:
...
if __name__ == '__main__':
    ...
    print("Select a scenario for main file:")
    print("S1: bioenergy_baseline")
    print("S1.1: bioproduct_baseline")
    print("S2: environ_baseline")
    print("S3: environ_only_baseline")
    print("S4: environ_baseline_TMDL")
    print("S5: environ_baseline_CRP")
    
    selected_scenario = input("Enter the scenario code: ")  # Prompt user for input

    # Validate the input and execute the selected scenario function
    if selected_scenario in ['S1', 'S1.1', 'S2', 'S3', 'S4', 'S5']:
        # Execute the selected scenario function
        scenario_functions[selected_scenario]()
    else:
        print("Invalid scenario code.")